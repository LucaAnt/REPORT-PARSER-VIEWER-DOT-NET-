<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="http://www.ni.com/TestStand" xmlns:vb_user="http://www.ni.com/TestStand/" id="TS17.0.0">
	...
	function GetArrayGraph(valueNodes, nMax, bDoDecimation, graphArrayObj, sDataLayout, sDataOrientation, arrayTable, addLabelAndRow)
	{
		var sRet = "";
		var inc = 1;
		var n = 0;
		...
		if (addLabelAndRow)
			sRet += "<td colspan='" + gSecondColumnSpan6 + "'>";
		sRet += "<span style='font-size:0.6em; white-space:nowrap;'>";
		...
		{
			sRet += "<object classid='clsid:1CB7D7EF-7ED3-4D6F-A2AC-F54DCA641862' id='TSGRAPH";
  ...
  // This function adds an array to the report as a graph (the elements will be numbers) and 
	...
	function AddArrayToReportAsGraph (propNodes, propName, propLabel, nLevel, flattenedStructure, objPath, arrayTable, addLabelAndRow)
	{
		var sRet = "";
		var nMax = 0;
		var bAddArray = true;
		...
		if(sDataLayout.toLowerCase() == "multipley" || sDataLayout.toLowerCase() == "singlex-multipley")
		{
			var dataOrientationNode = propNode.selectNodes(GRAPH_ATTRIBUTES_DATAORIENTATION_XPATH);	// DataOrientation attribute value is taken into consideration only when DataLayout attribute has a valid value
		...
		if (gIncludeArrayMeasurement != 0)
		{
			if (bAddArray)
			{
				var sArray = GetArrayGraph (valueNodes, nMax, bDoDecimation, graphArrayObj, sDataLayout, sDataOrientation, arrayTable, addLabelAndRow);
				// Add Label
				if (addLabelAndRow)
				{
					...
					if (!flattenedStructure)
					{
						if (valueNodes.length > 0)
						{
							sRet += "<span style='font-size:0.6em'>" + GetIndentationString(nLevel) + propLabel + graphArrayObj.SizeString + ":" + "</span>";
							sRet += "</td>\n"
						}
						else
						{
							sRet += "<span style='font-size:0.6em'>" + GetIndentationString(nLevel) + propLabel + "[0.." + "empty" + "]" + ":" + "</span>";
							sRet += "</td>\n"
						}
					}
					else
					{
						if (valueNodes.length > 0)
						{
							sRet += "<span style='font-size:0.6em'> <b>" + objPath + graphArrayObj.SizeString + ": </b> </span>";
							sRet += "</td>\n"
						}
						else
						{
							sRet += "<span style='font-size:0.6em'> <b>" + objPath + "[0.." + "empty" + "]" + ": </b> </span>";
							sRet += "</td>\n"
						}
					}
				}
	...
	function BeginTable()
	{
		...
		return	"\n<table style='margin-left:" + (GetIndentationLevel() * gIndentationWidth).toString(10) + "px;' border='1' cellpadding='2' cellspacing='0' width='70%'>" +
				...
				"<td colspan='4' align='center' style='width:33%'><span style='font-size:0.6em'><b>Limits</b></span></td>\n" +
				...
				"</tr>\n" +
				"<tr>" + 
				"<td style='width:10%' align='center'><span style='font-size:0.6em'><b>Nominal Value</b></span></td>\n" +
    ...
    //This function returns empty string if the serial number only contains white spaces, returns the string NONE if the serial number is empty or returns the serial number.
	...
	**/
	...
	<xsl:variable name="gFirstColumnSpan3" select="4"/>
	<xsl:variable name="gSecondColumnSpan1" select="2"/>
	...
	<xsl:variable name="gThirdColumnSpan1" select="2"/>
	<xsl:template match="/">
		<html>
			<head>
	...
	<xsl:template match="Prop[@Name='BatchSerialNumber']">
		<xsl:if test="Value != ''">
			<tr valign="top">
				...
				<td style='white-space:nowrap'>
					<xsl:variable name="batchSerialNumber" select="user:GetSerialNumber(Value)"/>
					<xsl:choose>
						<xsl:when test="$batchSerialNumber!=''">
					           <span style='font-size:0.6em'>
								<xsl:value-of select="$batchSerialNumber"/>
							</span>
						</xsl:when>
						<xsl:otherwise>
					     <span style='font-size:0.6em'><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></span>
						</xsl:otherwise>
					</xsl:choose>
				</td>
			</tr>
	...
	<xsl:template match="Prop[@Name='SerialNumber']">
		<tr valign="top">
			...
			<td style='white-space:nowrap'>
				<xsl:variable name="serialNumber" select="user:GetSerialNumber(Value)"/>
				<xsl:choose>
										...
										<xsl:if test="Prop[@Name='Limits']">
											...
											<td align="right">
												<xsl:if test="$areLimitsIncluded = 'True'">
													<xsl:if test="(Prop[@Name='Limits']/Prop[@Name='Nominal']/Value)">
														<span style="font-size:0.6em;white-space:nowrap">
															<xsl:choose>
																<xsl:when test="$reportOptions/Prop[@Name='UseLocalizedDecimalPoint']/Value = 'True' and $gLocalizedDecimalPoint != '.' ">
																	<xsl:value-of select="translate(Prop[@Name='Limits']/Prop[@Name='Nominal']/Value, '.', $gLocalizedDecimalPoint)"/>
																</xsl:when>
																<xsl:otherwise>
																	<xsl:value-of select="Prop[@Name='Limits']/Prop[@Name='Nominal']/Value"/>
																</xsl:otherwise>
															</xsl:choose>
														</span>
													</xsl:if>
													<xsl:if test="not(Prop[@Name='Limits']/Prop[@Name='Nominal']/Value)">
														<xsl:value-of select="$emptyCellValue" disable-output-escaping="yes"/>
													</xsl:if>
												</xsl:if>
													...
													<xsl:if test="(Prop[@Name='Limits']/Prop[@Name='Low']/Value)">
														<span style='font-size:0.6em;white-space:nowrap'>
															<xsl:variable name="low">
																<xsl:choose>
																	<xsl:when test="not(Prop[@Name='Limits']/Prop[@Name='ThresholdType'])">
																	...
																	<xsl:otherwise>
																		<xsl:variable name="isValidNumber">
																			<xsl:call-template name="IsValidNumber">
																				<xsl:with-param name="limitsPropNode" select="Prop[@Name='Limits']"/>
																			</xsl:call-template>
																		</xsl:variable>
																		<xsl:choose>
																			<xsl:when test="$isValidNumber = 'False'">
																				<xsl:variable name="thresholdTypeSymbol">
																					<xsl:choose>
																						<xsl:when test="translate(Prop[@Name='Limits']/Prop[@Name='ThresholdType']/Value, 'percentage', 'PERCENTAGE') = 'PERCENTAGE'"> %</xsl:when>
																						<xsl:when test="translate(Prop[@Name='Limits']/Prop[@Name='ThresholdType']/Value,'ppm', 'PPM') = 'PPM'"> PPM</xsl:when>
																					</xsl:choose>
																				</xsl:variable>
																				(Nominal - <xsl:value-of select="Prop[@Name='Limits']/Prop[@Name='Low']/Value"/><xsl:value-of select="$thresholdTypeSymbol"/>)
																			</xsl:when>
																			<xsl:otherwise>
																				<xsl:variable name="thresholdType" select="Prop[@Name='Limits']/Prop[@Name='ThresholdType']/Value"/>
																				<xsl:variable name="numericFormat">
																					<xsl:choose>
																						<xsl:when test="Prop[@Name='Limits']/Prop[@Name='Nominal']/@NumFmt">
																							<xsl:value-of select="Prop[@Name='Limits']/Prop[@Name='Low']/@NumFmt"/>
																						</xsl:when>
																						<xsl:otherwise/>
																					</xsl:choose>
																				</xsl:variable>
																				<xsl:variable name="nominal" select="Prop[@Name='RawLimits']/Prop[@Name='Nominal']/Value"/>
																				<xsl:variable name="lowValue" select="Prop[@Name='RawLimits']/Prop[@Name='Low']/Value"/>
																					<xsl:choose>
																					  <xsl:when test="Prop[@Name='RawLimits']/Prop[@Name='Nominal']/Value and Prop[@Name='RawLimits']/Prop[@Name='Low']/Value">
																						<xsl:value-of select="user:GetLimitThresholdValue($thresholdType, $numericFormat, $nominal, $lowValue, true())"/>    
																					  </xsl:when>
																					  <xsl:otherwise>
																						IND
																					  </xsl:otherwise>
															...
															<xsl:choose>
																<xsl:when test="$reportOptions/Prop[@Name='UseLocalizedDecimalPoint']/Value = 'True' and $gLocalizedDecimalPoint != '.' ">
																	<xsl:value-of select="translate($low, '.', $gLocalizedDecimalPoint)"/>
																</xsl:when>
																<xsl:otherwise>
												...
												<xsl:if test="$areLimitsIncluded = 'True'">
													<xsl:if test="(Prop[@Name='Limits']/Prop[@Name='High']/Value)">
														<span style='font-size:0.6em;white-space:nowrap'>
																	...
																	<xsl:otherwise>
																		...
																		<xsl:choose>
																			<xsl:when test="$isValidNumber = 'False'">
																				...
																				(Nominal + <xsl:value-of select="Prop[@Name='Limits']/Prop[@Name='High']/Value"/><xsl:value-of select="$thresholdTypeSymbol"/>)
																			</xsl:when>
																			<xsl:otherwise>
																				<xsl:variable name="thresholdType" select="Prop[@Name='Limits']/Prop[@Name='ThresholdType']/Value"/>
																				<xsl:variable name="numericFormat">
																					<xsl:choose>
																						<xsl:when test="Prop[@Name='Limits']/Prop[@Name='High']/@NumFmt">
																							<xsl:value-of select="Prop[@Name='Limits']/Prop[@Name='High']/@NumFmt"/>
																						</xsl:when>
																						<xsl:otherwise/>
																					</xsl:choose>
																				</xsl:variable>
																				<xsl:variable name="nominal" select="Prop[@Name='RawLimits']/Prop[@Name='Nominal']/Value"/>
																				<xsl:variable name="highValue" select="Prop[@Name='RawLimits']/Prop[@Name='High']/Value"/>
																					<xsl:choose>
																						  <xsl:when test="Prop[@Name='RawLimits']/Prop[@Name='Nominal']/Value and Prop[@Name='RawLimits']/Prop[@Name='High']/Value">
																							<xsl:value-of select="user:GetLimitThresholdValue($thresholdType, $numericFormat, $nominal, $highValue, false())"/>    
																						  </xsl:when>
																					  <xsl:otherwise>
																							IND
																					  </xsl:otherwise>
															...
															<xsl:choose>
																<xsl:when test="$reportOptions/Prop[@Name='UseLocalizedDecimalPoint']/Value = 'True' and $gLocalizedDecimalPoint != '.' ">
																	<xsl:value-of select="translate($high, '.', $gLocalizedDecimalPoint)"/>
																</xsl:when>
																<xsl:otherwise>
								...
								<xsl:if test="Prop[@Name='String']">
									<xsl:if test="Prop[@Name='Comp']">
										<xsl:if test="Prop[@Name='Limits']">
											<td align="left">
												<xsl:if test="$areMeasurementsIncluded = 'True'">
														...
														<xsl:if test="(Prop[@Name='String']/Value !='')">
															<xsl:variable name="stringDataValue" select="Prop[@Name='String']/Value"/>
															<xsl:choose>
																<xsl:when test="$gDisplayNewLineAndHTMLElementInStringProperty">
																	<xsl:value-of disable-output-escaping="yes" select="user:RemoveIllegalCharacters($stringDataValue)"/>
																</xsl:when>
																<xsl:otherwise>
																	<xsl:call-template name="RemoveWhiteSpaces">
																		<xsl:with-param name="inStr" select="$stringDataValue"/>
																	</xsl:call-template>
																</xsl:otherwise>
															</xsl:choose>
														</xsl:if>
											...
											<!-- String Value Tests cannot have Nominal Value -->
											<td>
												<xsl:value-of select="$emptyCellValue" disable-output-escaping="yes"/>
											</td>
											<td align="left">
												<xsl:if test="$areLimitsIncluded = 'True'">
														...
														<xsl:if test="(Prop[@Name='Limits']/Prop[@Name='String']/Value != '')">
															<xsl:variable name="stringLimitValue" select="Prop[@Name='Limits']/Prop[@Name='String']/Value"/>
															<xsl:choose>
																<xsl:when test="$gDisplayNewLineAndHTMLElementInStringProperty">
																	<xsl:value-of disable-output-escaping="yes" select="user:RemoveIllegalCharacters($stringLimitValue)"/>
																</xsl:when>
																<xsl:otherwise>
																	<xsl:call-template name="RemoveWhiteSpaces">
																		<xsl:with-param name="inStr" select="$stringLimitValue"/>
																	</xsl:call-template>
																</xsl:otherwise>
															</xsl:choose>
														</xsl:if>
		...
		<xsl:if test="Prop[@Name='ResultList']">
			<xsl:if test="Prop[@Name='ResultList'] and Prop[@HBound != '[]']">
				...
				<xsl:variable name="blockLevel">
					<xsl:value-of select="Prop[@Name='ResultList']/Value[@ID]/Prop[@Type='TEResult']/Prop[@Name='TS']/Prop[@Name='BlockLevel']/Value"/>
				</xsl:variable>
				<xsl:if test="$blockLevel = 0">
					<xsl:value-of disable-output-escaping="yes" select="user:BeginTable()"/>
				</xsl:if>
				<xsl:apply-templates select="Prop[@Name='ResultList']/Value[@ID]/Prop[@Type='TEResult']">
					<xsl:with-param name="reportOptions" select="$reportOptions"/>
									...
									<xsl:otherwise>
										...
										<xsl:choose>
											...
											<xsl:otherwise>
												...
												<xsl:variable name="numericFormat">
													<xsl:choose>
														<xsl:when test="*/Prop[@Name='Limits']/Prop[@Name='Low']/@NumFmt">
															<xsl:value-of select="*/Prop[@Name='Limits']/Prop[@Name='Low']/@NumFmt"/>
														</xsl:when>
														<xsl:otherwise/>
													</xsl:choose>
	...
	<xsl:template match="Report[@Type='Batch']">
		<xsl:variable name="reportOptions" select="Prop[@Name='ReportOptions']"/>
		<xsl:value-of select="user:InitFlagGlobalVariables(Prop[@Name='ReportOptions'])"/>
		<xsl:variable name="batchSerialNumber" select="user:GetSerialNumberFromText(string(@BatchSerialNumber))"/>
		<h3>
			<span style='font-size:0.7em'>
				<xsl:value-of select="@Title"/>
			</span>
	...
	<xsl:template name="GetComparisonTypeText">
		...
		<xsl:choose>
			...
			<xsl:when test="$compText='EQT'">
				<xsl:text disable-output-escaping="yes">EQT (== +/-)</xsl:text>
			</xsl:when>
						...
						<xsl:choose>
							<xsl:when test="$propNode/@Type = 'Array'">
								...
								<xsl:if test="$arrayElemPropTypeName != 'NI_LimitMeasurement'">
									<xsl:choose>
										<xsl:when test="@Name = 'ListOfUpdatedProperties' and ../../../ArrayElementPrototype/@TypeName = 'NI_PropertyLoaderResult'">
											<xsl:if test="count(./Value) != 0">
												<xsl:call-template name="PutListOfUpdatedProperties">
													<xsl:with-param name="nLevel" select="$nLevel"/>
													<xsl:with-param name="reportOptions" select="$reportOptions"/>
												</xsl:call-template>
											</xsl:if>
										...
										<xsl:otherwise>
											...
											<xsl:choose>
												...
												<xsl:when test="($numDimensions - 1) &lt;= 2 and $propNode/@ElementType = 'Number' and $includeArrayMeasurement = 2 
													and $propNodeRepresentation = 'DBL' and $isDecimal='true' and count($propNode/Value)>0">
													...
													<xsl:value-of select="user:AddArrayToReportAsGraph($propNode, $propNode/@Name, string($propLabel), $nLevel, boolean($flattenedStructure), string($currentObjPath), string($arrayTable), boolean(true()))" disable-output-escaping="yes"/>
									...
									<xsl:when test="./Value">
										...
										<xsl:choose>
											...
											<xsl:when test="($propNode/@Name = 'String' and $parentPropName = 'Limits' and $propNode/@Type = 'String') or
												($propNode/@Name = 'ThresholdType' and $parentPropName = 'Limits' and $propNode/@Type = 'String') or
												($propNode/@Name = 'Nominal' and $parentPropName = 'Limits' and $propNode/@Type = 'Number') or
							...
							<xsl:when test="$propNode/Prop[@Name='File']/Value != '' and $propNode/Prop[@Name='File']/Attributes/Prop[@Name='TestStand']/Prop[@Name='Hyperlink' and @Type='Boolean']/Value='True'">
								<a>
									...
									<span style='font-size:0.6em'>
										<xsl:choose>
											<xsl:when test="$gDisplayNewLineAndHTMLElementInStringProperty">
												<xsl:value-of disable-output-escaping="yes" select="user:RemoveIllegalCharacters($propNode/Prop[@Name='File']/Value)"/>
											</xsl:when>
											<xsl:otherwise>
												<xsl:call-template name="RemoveWhiteSpaces">
	...
	<!--Template to add list of updated properties in a property loader step-->
	<xsl:template name="PutListOfUpdatedProperties">
		<xsl:param name="nLevel"/>
		<xsl:param name="reportOptions"/>
		
		<tr>
			<td valign="middle" style='white-space:nowrap' colspan='{$gSecondColumnSpan7}'>
				<span style='font-size:0.6em'>
						<xsl:call-template name="GetIndentationString">
							<xsl:with-param name="nLevel" select="$nLevel"/>
						</xsl:call-template>ListOfUpdatedProperties:
				</span>
			</td>
		</tr>
		
		<tr>
			<td align="center" valign="middle" style='white-space:nowrap' colspan='{$gFirstColumnSpan3}'>
				<span style='font-size:0.6em'><b>Property Lookup</b></span>
			</td>
			<td align="center" valign="middle" style='white-space:nowrap' colspan='{$gSecondColumnSpan1}'>
				<span style='font-size:0.6em'><b>OldValue</b></span>
			</td>
			<td align="center" valign="middle" style='white-space:nowrap' colspan='{$gThirdColumnSpan1}'>
				<span style='font-size:0.6em'><b>NewValue</b></span>
			</td>
		</tr>
		
		<xsl:for-each select="./Value/Prop">
			<tr>
				<td valign="middle" style='white-space:nowrap' colspan='{$gFirstColumnSpan3}'>
					<span style='font-size:0.6em'>
						<xsl:call-template name="GetIndentationString">
							<xsl:with-param name="nLevel" select="$nLevel + 1"/>
						</xsl:call-template>
						<xsl:value-of select="Prop[@Name='PropertyLookup']/Value"/>
					</span>
				</td>
				<td valign="middle" style='white-space:nowrap' colspan='{$gSecondColumnSpan1}'>
					<xsl:for-each select="Prop[@Name='OldValue']">
						<xsl:call-template name="PutValueOfUpdatedProperty">
							<xsl:with-param name="nLevel" select="$nLevel"/>
							<xsl:with-param name="propNode" select="."/>
							<xsl:with-param name="reportOptions" select="$reportOptions"/>
						</xsl:call-template>
				...
				<td align="center" valign="middle" style='white-space:nowrap' colspan='{$gThirdColumnSpan1}'>
					<xsl:for-each select="Prop[@Name='NewValue']">
						<xsl:call-template name="PutValueOfUpdatedProperty">
							<xsl:with-param name="nLevel" select="$nLevel"/>
							<xsl:with-param name="propNode" select="."/>
							<xsl:with-param name="reportOptions" select="$reportOptions"/>
	...
	<!--Template to add one updated property value-->
	<xsl:template name="PutValueOfUpdatedProperty">
		<xsl:param name="nLevel"/>
		<xsl:param name="reportOptions"/>
		<xsl:param name="propNode" select="."/>
		<span style='font-size:0.6em'>
			<xsl:choose>
				<xsl:when test="$propNode/@Type != 'Array'">
					<xsl:value-of select="$propNode/Value"/>
				...
				<xsl:otherwise>
					...
					<xsl:choose>
						...
						<xsl:when test="($numDimensions - 1) &lt;= 2 and $propNode/@ElementType = 'Number' and $includeArrayMeasurement = 2 
								...
								<xsl:choose>
									<xsl:when test="$gGraphControlInstalled != 1">
										<xsl:call-template name="AddArrayToReportAsTable">
											<xsl:with-param name="propNode" select="$propNode"/>
											<xsl:with-param name="propName" select="$propNode/@Name"/>
											<xsl:with-param name="propLabel" select="''"/>
											<xsl:with-param name="nLevel" select="$nLevel"/>
											<xsl:with-param name="reportOptions" select="$reportOptions"/>
											<xsl:with-param name="flattenedStructure" select="false()"/>
							...
							<xsl:value-of select="user:AddArrayToReportAsGraph($propNode, $propNode/@Name, string(''), $nLevel, boolean(false()), string(''), string($arrayTable), boolean(false()))" disable-output-escaping="yes"/>
						</xsl:when>
						<!-- For all other cases the array will be added as a table -->
						<xsl:otherwise>
							<xsl:call-template name="AddArrayToReportAsTable">
						...
						<span style='font-size:0.6em'>
							<a>
								...
								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:call-template name="RemoveWhiteSpaces">
											<xsl:with-param name="inStr" select="$propValue"/>
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>
							</a>
							...
							<td valign="top">
								<span style='font-size:0.6em'>
									...
									<xsl:choose>
										...
										<xsl:otherwise>
											<b>
												<xsl:value-of select="$objectPath" disable-output-escaping="no"/>
