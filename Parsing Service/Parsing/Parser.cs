...
namespace Parsing_Service.Parsing
{
    static class Parser
    {
        ...
        //MAIN EXTERNAL API
        public static void setParserConfiguration(String directory,int mode, DateTime lastParse, List<String> filters, BackgroundWorker bw)
        {
            reset();
            _PARSEMODE = mode;
        ...
        private static void TransformXMLToHTML(string XmlPath, string xsltPath)
        {
            XslCompiledTransform myXslTransform = new XslCompiledTransform();
            try
            {
                myXslTransform.Load(xsltPath, XsltSettings.TrustedXslt, new XmlUrlResolver());
            }
            catch (Exception e)
            {
            ...
            myXslTransform.Transform(XmlPath, Path.Combine(Path.GetDirectoryName(XmlPath), Path.GetFileName(Path.ChangeExtension(XmlPath, ".html"))));
        }
